...
def main():
    ...
    parser.add_argument("--adapter", type=str, default="adapters/dpo_adapter", help="Adapter path")
    parser.add_argument("--input", type=str, default="sft_data/test.jsonl", help="Input JSONL file with prompts")
    parser.add_argument("--output", type=str, default="eval_outputs/dpo_eval_outputs.jsonl", help="Output JSONL file")
    ...
    with open(args.output, "w") as f_out:
        for item, prompt_text in tqdm(prompts, desc="Generating"):
            ...
            output_item = {"prompt": prompt_text, "dpo_model_response": response.strip(), "base_model_response": response.strip()}
